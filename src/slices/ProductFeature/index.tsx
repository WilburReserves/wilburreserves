...
const ProductFeature: FC<ProductFeatureProps> = async ({ slice }) => {
  ...
  const embers = isFilled.contentRelationship(slice.primary.ember) ? await client.getByID<Content.EmbersDocument>(slice.primary.ember.id) : null;

  const formattedPrice = formatPrice(embers?.data.price);
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-3 lg:grid-rows-[auto, auto]">
        ...
        <PrismicNextImage field={slice.primary.mage} className="h-auto w-full object-cover"/>
        </FadeIn>
        ...
        >
          ...
          <PrismicImage field={embers?.data.bottle_image}
          ...
          <h3 className="font-display text-4xl">
            <PrismicText field={embers?.data.title} fallback="Embers"/>
          </h3>
          <p className="mt-2 text-gray-300"> 
            Classical Edition
          </p>
          <ButtonLink document={embers} variant="Secondary" className="mt-6">Pre-order</ButtonLink>
            </div>
